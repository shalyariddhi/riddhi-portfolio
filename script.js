document.addEventListener('DOMContentLoaded', () => {
  ...
  // ==========================================
  // 8. Contact Form → Email Notification (FormSubmit)
  // ==========================================
  const contactForm = document.querySelector('#contact-form');
  const formMsg = document.querySelector('#form-msg');
  const RECIPIENT_EMAIL = 'riddhishalya2506@gmail.com';
  const FORMSUBMIT_URL = `https://formsubmit.co/ajax/${encodeURIComponent(RECIPIENT_EMAIL)}`;

  if (contactForm) {
    contactForm.addEventListener('submit', (e) => {
      e.preventDefault();

      const honeypot = document.querySelector('#honey').value.trim();
      if (honeypot) return;
      ...
      .then(async (response) => {
        ...
        if (!response.ok) {
          ...
          if (response.status === 403 || errorMsg.includes('activate') || errorMsg.includes('confirm')) {
      ...
      .then(() => {
        ...
        showMsg('Message sent! You will receive an email notification shortly.', 'success');
        contactForm.reset();
      })
      .catch((error) => {
        btnText.textContent = originalText;
        ...
        if (error.message === 'activation_required') {
          showMsg(
            `Almost there! Check ${RECIPIENT_EMAIL} for a FormSubmit activation email and click the confirmation link. After that, every message from this form will arrive in your inbox instantly.`,
            'success'
          );
          contactForm.reset();
        } else {
          showMsg(
            'Could not send the message. If this is your first time, check your email to activate FormSubmit, then try again.',
            'error'
          );
        }
      });
